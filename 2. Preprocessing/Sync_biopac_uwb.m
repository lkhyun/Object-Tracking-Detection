...
sub_n = 2  % sub 1~60 숫자 바꾸면서 데이터 추출, s1,2,3 bed radar 데이터 측정 이상 44부터 추가 진행!!!

%%%%%%%%%%%%%%%%%%%%%%% UWB raw data extraction %%%%%%%%%%%%%%%%%%%%%%%%%%%

dir_name = ['C:\Users\dlrkd\Desktop\hai-lab\[2401~2402] UWB_Biopac_Bed dataset\', sprintf('%s',sub_name{sub_n})];

current_dir = dir([sprintf('%s', dir_name), '\Move_Radar']);
idx1 = find(~cellfun(@isempty, strfind({current_dir.name}, 'computer')));
...
for iter = 1:num_iterations
    ...
    cd(sprintf('%s\\Move_Radar\\%s', dir_name, current_radarpath))
...
Fs_uwb = 17;
rawdata_computer = UWB_synchronized_data.UWB_rawdata_computer(100:459, :);
rawdata_tv = UWB_synchronized_data.UWB_rawdata_tv(100:459, :); 
...
%% mat 파일로 저장
com_startpoint = 155; % 시작 지점 설정
tv_startpoint = 578; % 시작 지점 설정
...
UWBFilteredData.com = plot_com_smooth(:,com_startpoint:com_startpoint + 4283);
UWBFilteredData.tv = plot_tv_smooth(:,tv_startpoint:tv_startpoint + 4283);
