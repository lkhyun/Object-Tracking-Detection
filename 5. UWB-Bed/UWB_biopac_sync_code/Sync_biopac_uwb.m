...
cd('C:\Users\fgh75\OneDrive - 광운대학교\바탕 화면\학부연구생\rader\ex_dataset\UWB_biopac_sync_code\[Lib] x64_MSVCR141');
...
sub_name = dir('C:\Users\fgh75\OneDrive - 광운대학교\바탕 화면\학부연구생\rader\ex_dataset\[2401~2402] UWB_Biopac_Bed dataset');
names = {sub_name.name};
sub_name= natsort(char(names)); 
Fs_uwb = 17;

sub_n = 12  % sub 1~60 숫자 바꾸면서 데이터 추출, s1,2,3 bed radar 데이터 측정 이상

%%%%%%%%%%%%%%%%%%%%%%% UWB raw data extraction %%%%%%%%%%%%%%%%%%%%%%%%%%%

dir_name = ['C:\Users\fgh75\OneDrive - 광운대학교\바탕 화면\학부연구생\rader\ex_dataset\[2401~2402] UWB_Biopac_Bed dataset\', sprintf('%s',num2str(sub_n))];
 
current_dir = dir([sprintf('%s', dir_name), '\Bed_Radar']);

sprintf('%s',dir_name)

idx = find(contains({current_dir.name}, 'bed'));

fname = current_dir(idx).name
cd(sprintf('%s\\Bed_Radar\\%s', dir_name, fname))

current_dir = pwd;
pattern = 'xethru_datafloat';
FileList = dir([current_dir, '\*', pattern, '*']);
...
cd(['C:\Users\fgh75\OneDrive - 광운대학교\바탕 화면\학부연구생\rader\ex_dataset\[2401~2402] UWB_Biopac_Bed dataset\', sprintf('%s',num2str(sub_n))]);
